...
describe ActiveNoSql::Relation do
  ...
  describe '#filter' do
	  ...
	  it "do merge single field, multiple value filters" do
	    test = ActiveNoSql::Relation.new(Item).filter(:"provider.name"=>'kittens').filter(:"provider.name"=>'cats')
	  ...
	  it "do a multiple field, single value filter" do
	    test = ActiveNoSql::Relation.new(Item).filter(:object=>'kittens',:isShownAt=>'bears')
	  ...
	  it "should keep scope distinct from spawned Relations" do
	    test = ActiveNoSql::Relation.new(Item).filter(:"provider.name"=>'kittens')
	    test.where(:q=>'cats')
