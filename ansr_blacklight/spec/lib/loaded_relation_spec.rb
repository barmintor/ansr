...
describe Ansr::Blacklight::Relation do
  ...
  let(:solr) do
    solr = double('Solr')
    allow(solr).to receive(:send_and_receive).and_return(eval(mock_query_response))
    solr
  end
  let(:model) do
    Object.const_set('LoadTestModel', Class.new(TestModel))
    LoadTestModel.solr = solr
    LoadTestModel
  end

  after do
    Object.send(:remove_const, :LoadTestModel) if Object.const_defined? :LoadTestModel
  end
  subject do
    relation = Ansr::Blacklight::Relation.new(model, Ansr::Arel::BigTable.new(model))
    relation.load
    relation
  end
  ...
  describe 'pagination' do
    ...
    it "should provide kaminari pagination helpers" do
      allow(solr).to receive(:send_and_receive).and_return(eval(mock_query_response))
      expect(subject.total_count).to eq(subject.count)
      expect(subject.next_page).to eq(subject.current_page + 1)
      expect(subject.prev_page).to eq(nil)
      if Kaminari.config.respond_to? :max_pages
        expect(subject.max_pages).to be_nil
      end
      expect(subject).to be_a_kind_of Kaminari::PageScopeMethods
    end
  end

  it 'should create a valid response class' do
    expect(subject).to respond_to(:response)
    expect(subject.docs).to have(11).docs
    expect(subject.params[:echoParams]).to eq 'EXPLICIT'
  ...
  it "should provide a model name helper" do
    expect(subject.model_name).to eq LoadTestModel.name
  end
